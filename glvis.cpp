...
void ExecuteScriptCommand(GLVisWindow* wnd)
{
   ...
   StreamState& state = wnd->getStreamState();
   ...
   while (!done_one_command)
   {
      ...
      else if (word == "solution" || word == "mesh" || word == "psolution")
      {
         ...
         if (state.SetNewMeshAndSolution(std::move(new_state), vs))
         {
            wnd->MyExpose();
         }
         else
      ...
      {
         scr >> state.keys;
         cout << "Script: keys: '" << state.keys << "'" << endl;
         // SendKeySequence(keys.c_str());
         wnd->CallKeySequence(state.keys.c_str());
         wnd->MyExpose();
      }
